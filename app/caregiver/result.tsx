...
function mapItemFromAnalyze(it: any): Item {
  return {
    ...
    quantity: it.quantity ?? "依醫囑",
    time: toArray(it.usage_zh ?? it.usage ?? it.time_of_day ?? it.time),
    note: it.note_zh ?? "",
  };
}
...
function makePrescriptionId(date = new Date()) {
  ...
  async function handlePrimaryAction() {
    if (isDraftMode) {
      ...
      try {
        ...
        const prescriptionId = makePrescriptionId();
        ...
        console.log("[save] caregiverUid =", user.uid);
        console.log("[save] careTargetId =", activeCareTargetId);
        console.log("[save] items committed =", medicines.length);
